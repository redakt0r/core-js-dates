...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const MILLISECONDS_IN_DAY = 1000 * 60 * 60 * 24;
  const dayOne = new Date(dateStart);
  const dayTwo = new Date(dateEnd);
  const diff = dayTwo - dayOne + MILLISECONDS_IN_DAY;
...
function getCountWeekendsInMonth(month, year) {
  ...
  let saturdaysAndSundays = 0;
  for (let i = 1; i <= daysInMonth; i += 1) {
    const currentDay = new Date(year, month - 1, i).getDay();
    if (currentDay === 6 || currentDay === 0) saturdaysAndSundays += 1;
  }
...
function getWeekNumberByDate(date) {
  ...
  const diff = (day - firstDayOfYear) / MILLISECONDS_IN_DAY;
